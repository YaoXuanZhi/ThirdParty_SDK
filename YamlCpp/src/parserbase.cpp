...
namespace YAML
{
	...
	ParserBase::ParserBase(std::istream& in)
	{
		Load(in);
	}
	...
	ParserBase::operator bool() const
	{
		return m_pScanner.get() && !m_pScanner->empty();
	}

	void ParserBase::Load(std::istream& in)
	{
	...
	bool ParserBase::GetNextDocument(Node& document)
	{
		if(!m_pScanner.get())
			return false;
	...
	void ParserBase::ParseDirectives()
	{
	...
	void ParserBase::HandleDirective(const Token& token)
	{
		if(token.value == "YAML")
	...
	void ParserBase::HandleYamlDirective(const Token& token)
	{
	...
	void ParserBase::HandleTagDirective(const Token& token)
	{
	...
	void ParserBase::PrintTokens(std::ostream& out)
	{
		if(!m_pScanner.get())
			return;
