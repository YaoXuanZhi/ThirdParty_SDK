		    ...
		    int eflags)
{
  ...
  extra_nmatch = (unsigned int)((nmatch > preg->re_nsub) ? nmatch - (preg->re_nsub + 1) : 0);
  nmatch -= extra_nmatch;
  ...
  err = re_string_allocate (&mctx.input, string, length, (int)(dfa->nodes_len + 1),
	  ...
	  regmatch_t *pmatch, int fl_backtrack)
{
  ...
  for (idx = pmatch[0].rm_so; idx <= pmatch[0].rm_eo ;)
      ...
      if (idx == pmatch[0].rm_eo && cur_node == mctx->last_node)
	{
      ...
      cur_node = proceed_next_node (mctx, (int)nmatch, pmatch, &idx, cur_node,
				    &eps_via_nodes, fs);
