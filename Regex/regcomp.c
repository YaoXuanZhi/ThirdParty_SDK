		     ...
		     reg_syntax_t syntax)
{
  ...
  err = re_string_construct (&regexp, pattern, (int)length, preg->translate,
			     syntax & RE_ICASE, dfa);
  if (BE (err != REG_NOERROR, 0))
    {
...
create_initial_state (re_dfa_t *dfa)
{
  ...
  dfa->init_node = first;
  err1 = re_node_set_init_copy (&init_nodes, dfa->eclosures + first);
  if (BE (err1 != REG_NOERROR, 0))
    return err1;
	...
	if (type == OP_BACK_REF)
	  {
	    ...
	    if (!re_node_set_contains (&init_nodes, dest_idx))
	      {
		reg_errcode_t err2 = re_node_set_merge (&init_nodes,
						       dfa->eclosures
						       + dest_idx);
		if (err2 != REG_NOERROR)
		  return err2;
  ...
  dfa->init_state = re_acquire_state_context (&err1, dfa, &init_nodes, 0);
  /* We don't check ERR here, since the initial state must not be NULL.  */
  if (BE (dfa->init_state == NULL, 0))
    return err1;
  if (dfa->init_state->has_constraint)
    {
      dfa->init_state_word = re_acquire_state_context (&err1, dfa, &init_nodes,
						       CONTEXT_WORD);
      dfa->init_state_nl = re_acquire_state_context (&err1, dfa, &init_nodes,
						     CONTEXT_NEWLINE);
      dfa->init_state_begbuf = re_acquire_state_context (&err1, dfa,
							 &init_nodes,
							 CONTEXT_NEWLINE
							 | CONTEXT_BEGBUF);
...
{
  int ret;
  unsigned int src, idx1;
  for (idx1 = 0; idx1 < dfa->nodes_len; ++idx1)
    re_node_set_init_empty (dfa->inveclosures + idx1);
  ...
  for (src = 0; src < dfa->nodes_len; ++src)
	...
	{
	  ret = re_node_set_insert_last (dfa->inveclosures + elems[idx2], src);
	  if (BE (ret == -1, 0))
	    return REG_ESPACE;
	}
    }
...
calc_eclosure (re_dfa_t *dfa)
{
  ...
  for (node_idx = 0; ; ++node_idx)
	...
	continue;
      ...
      err = calc_eclosure_iter (&eclosure_elem, dfa, (int)node_idx, 1);
      if (BE (err != REG_NOERROR, 0))
	return err;
	      ...
	      re_token_t *token, reg_syntax_t syntax, reg_errcode_t *err)
{
  ...
  tempindex.value = elem->token.opr.idx;

  if (elem->token.type == SUBEXP)
	  postorder(elem, mark_opt_subexp, (void *)&tempindex);
...
{
  reg_intex_t *tempindex = (reg_intex_t*)extra;
  int idx = tempindex->value;
